...
export function deliveryEmailHtml(args: {
  ...
  const cta = args.downloadUrl
    ? `<a href="${args.downloadUrl}" style="display:inline-block;background:#4f46e5;color:#fff;padding:12px 22px;border-radius:12px;text-decoration:none;font-weight:700">Download your file</a>
       <p style="color:#64748b;font-size:12px">This secure link expires on ${args.expiresAt?.toUTCString()}.</p>`
    : `<a href="${args.meetingLink}" style="display:inline-block;background:#4f46e5;color:#fff;padding:12px 22px;border-radius:12px;text-decoration:none;font-weight:700">Join your session</a>
  ...
  return `<div style="font-family:Inter,Arial,sans-serif;background:#0b0f19;padding:32px">
    <div style="max-width:520px;margin:auto;background:#111827;border:1px solid #1f293d;border-radius:20px;padding:28px;color:#e2e8f0">
      <h1 style="font-size:18px;margin:0 0 8px">Thanks for your purchase, ${args.customerName}!</h1>
      <p style="font-size:14px;color:#94a3b8;margin:0 0 20px">
        Your order for <b style="color:#fff">${args.productTitle}</b> from ${args.creatorName} is confirmed.
      </p>
      ${cta}
